...
impl App {
        ...
        if let MessageKind::Text { ref data, .. } = message.kind {
            ...
            if let Some(cap) = re.captures(data.as_str()) {
                if cap["botname"].len() <= 0 || &cap["botname"] == env::var("BOT_NAME").expect("BOT_NAME not set") {
                    self.cmds[&cap["command"]].execute(api, &message, &cap["arg"]).await?
                }
            }
        };
        Ok(())
    ...
    pub async fn run(&self, api: Api, message: Message) -> Result<(), Error> {
        if let Err(e) = self.router(api, message).await {
            println!("{:?}", e);
        }
        Ok(())
    }
