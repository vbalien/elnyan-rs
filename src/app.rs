    ...
    pub async fn message(
        ...
        if let MessageKind::Text { ref data, .. } = message.kind {
            ...
            if let Some(cap) = re.captures(data.as_str()) {
                ...
                if (botname.is_empty() || botname == ctx.botname) && self.cmds.contains_key(command)
                {
                    self.cmds
                        .get(command)
                        .unwrap()
                        .do_command(ctx, &message, arg)
                        .await?
                    ...
                    && !self.cmds.contains_key(command)
                {
                    self.cmds
                        .get("_")
                        .unwrap()
                        .do_command(ctx, &message, command)
                        .await?
                }
            }
        };
    ...
    pub async fn callback(
        ...
        match &cbq.data {
            Some(data) => match data.as_str() {
                "selfdel" => {
                    ctx.api
                        .send(&cbq.message.as_ref().unwrap().delete())
                        .await?
                }
                _ => {
